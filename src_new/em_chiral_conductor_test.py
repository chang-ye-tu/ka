...
for d in [[1, 0],]:   # observed direction   
    ...
    for n in [8, 16, 32, 64, 128]:
        ...
        for l in arange(2*n):
            ...
            u[2*l + 0] =  - 1.j * (c_er * hankel1(0, gamma_er * norm) - c_el * hankel1(0, gamma_el * norm))
            u[2*l + 1] =  - 1.j * ( - c_er * d_nu * hankel1(1, gamma_er * norm) - c_el * d_nu * hankel1(1, gamma_el * norm))

        E = zeros((4*n, 4*n), dtype=complex)
        for l in arange(2*n):
            E[2*l + 0, 2*l + 0] = (1.j * gamma_er + 1.j * gamma_el) * 1
            E[2*l + 0, 2*l + 1] = 0
            E[2*l + 1, 2*l + 0] = 0
            E[2*l + 1, 2*l + 1] = (1.j / (2 * gamma_er) + 1.j / (2 * gamma_el)) * 1

        A = zeros((4*n, 4*n), dtype=complex)
        for l in arange(2*n):
            for m in arange(2*n):
                A[2*l + 0, 2*m + 0] = 1.j * gamma_er * (R(l - m) * L1(pi*l/n, pi*m/n, gamma_er) + pi/n * L2(pi*l/n, pi*m/n, gamma_er)) + 1.j * gamma_el * (R(l - m) * L1(pi*l/n, pi*m/n, gamma_el) + pi/n * L2(pi*l/n, pi*m/n, gamma_el))
                A[2*l + 0, 2*m + 1] = 1.j * (R(l - m) * M1(pi*l/n, pi*m/n, gamma_el) + pi/n * M2(pi*l/n, pi*m/n, gamma_el)) / 2 - 1.j * (R(l - m) * M1(pi*l/n, pi*m/n, gamma_er) + pi/n * M2(pi*l/n, pi*m/n, gamma_er)) / 2
                A[2*l + 1, 2*m + 0] = 1.j * (R(l - m) * N1(pi*l/n, pi*m/n, gamma_er) + pi/n * N2(pi*l/n, pi*m/n, gamma_er)) - 1.j * (R(l - m) * N1(pi*l/n, pi*m/n, gamma_el) + pi/n * N2(pi*l/n, pi*m/n, gamma_el))
                A[2*l + 1, 2*m + 1] =  - 1.j * (R(l - m) * Ls1(pi*l/n, pi*m/n, gamma_er) + pi/n * Ls2(pi*l/n, pi*m/n, gamma_er)) / (2 * gamma_er) - 1.j * (R(l - m) * Ls1(pi*l/n, pi*m/n, gamma_el) + pi/n * Ls2(pi*l/n, pi*m/n, gamma_el)) / (2 * gamma_el)

        z = linalg.solve(E + A, u)
        
        for l in arange(2*n):
            Q_er_inf += (z[2*l+1] * (1.j * xp(l*pi/n)) - 2 * z[2*l] * gamma_er * (gamma_er * (d[0] * x2p(l*pi/n) - d[1] * x1p(l*pi/n)))) * exp(-1.j * gamma_er * (d[0] * x1(l*pi/n) + d[1] * x2(l*pi/n)))
            Q_el_inf += (z[2*l+1] * (1.j * xp(l*pi/n)) + 2 * z[2*l] * gamma_el * (gamma_el * (d[0] * x2p(l*pi/n) - d[1] * x1p(l*pi/n)))) * exp(-1.j * gamma_el * (d[0] * x1(l*pi/n) + d[1] * x2(l*pi/n)))
    ...
    open("data/em_chiral_conductor.data", "w").write(s1 + '\n' + s2)
