...
for d in [[1, 0],]:   # observed direction   
    ...
    for n in [8, 16, 32, 64, 128]:
        ...
        for l in arange(2*n):
            ...
            u[4*l + 0] = -2 * k * exp(1j * k * xdp) * d_nu 
            u[4*l + 1] = -2 * k * exp(1j * k * xdp) * d_nu
            u[4*l + 2] = -2 * k * exp(1j * k * xdp) * d_nu 
            u[4*l + 3] = -2 * k * exp(1j * k * xdp) * d_nu
        ...
        for l in arange(2*n):
            Q_er_inf += (z[4*l+3] * (1.j * xp(l*pi/n)) + (k_e * (d[0] * x2p(l*pi/n) - d[1] * x1p(l*pi/n))) * (k_e * z[4*l+1] * (rho + delta) + k_e * z[4*l] * (rho - delta))) * exp(-1.j * gamma_er * (d[0] * x1(l*pi/n) + d[1] * x2(l*pi/n)))
            Q_el_inf += (z[4*l+2] * (1.j * xp(l*pi/n)) + (k_e * (d[0] * x2p(l*pi/n) - d[1] * x1p(l*pi/n))) * (k_e * z[4*l] * (rho + delta) + k_e * z[4*l+1] * (rho - delta))) * exp(-1.j * gamma_el * (d[0] * x1(l*pi/n) + d[1] * x2(l*pi/n)))
